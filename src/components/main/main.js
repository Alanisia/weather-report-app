...
export default class Main extends React.Component {
  ...
  // } else {
  // let weatherInfo = this.state.weatherInfo.data;
  // let t = {
  ...
  // NOTE: Replace codes upon here when test UI successfully.
  render() {
    const t = FakeDatas.default.weather;
    const h = FakeDatas.default.weatherHours;
    const w = FakeDatas.default.weather7Days;
    const d = FakeDatas.default.details;
    return (
      <div>
        <TemperatureHead temp={t} /><hr />
        <TemperatureHours weatherHours={h} /><hr />
        <Temperature7Days weather7Days={w} /><hr />
        <TemperatureDetails weatherDetails={d} />
      </div>
    );
  }
}